...
public class WallJumpAgent : Agent
{
    ...
    public float dist;
    public float last_distance = 100000;
    ...
    public bool DoGroundCheck(bool smallCheck)
    {
        ...
        else
        {
            ...
            //if(hit.collider != null && hit.collider.CompareTag("wall") || hit.collider.CompareTag("block"))
            //{
                //AddReward(0.1f);
            //}
    ...
    public void MoveAgent(ActionSegment<int> act)
    {
        ...
        dist = Vector3.Distance(this.transform.localPosition, new Vector3(6.37f,0.49f,2.93f));
    ...
    {
        if (collider.transform.tag.ToLower() == "wall" || collider.transform.tag.ToLower() == "block")
        {
           RaycastHit hit;
            Physics.Raycast(transform.position + new Vector3(0, -1f, 0), -Vector3.up, out hit,
                1f);
            Debug.DrawRay(transform.position + new Vector3(0, -1f, 0), -Vector3.up, Color.green);
            if (hit.collider != null)
            {
                if (hit.collider.CompareTag("wall") || hit.collider.CompareTag("block"))
                {
                    AddReward(0.01f);
                }else if(collider.transform.tag.ToLower() == "wall")
                {
                    AddReward(-0.1f);
                    EndEpisode();
